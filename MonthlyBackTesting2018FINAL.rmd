---
title: "BackTesting2018_monthly_sharperatio"
...
for (j in 1:12) {    # for loop for months
TickerList.2018 <- paste(TickerList[1:100,j], sep = "") # Choose S&P 100

ClosingPricesRead <- NULL #Find the adjusted prices for each company in S&P 100
for (Ticker in TickerList.2018){
...
a <- (j-1)*21 # each month has 21 trading days ( there is no trading in weekends)
...
numtick <- length(TickerList.2018) #number of ticker we always be 100 
...
rownames(TickerInfo) <- TickerList.2018
...
Allocation.Matrix.2018 <- list() 
Monthly.Stock.Returns.2018 <- list()
...
for(j in 1:12){ #12 represents the months of the year
...
for(k in 1:10) {
  Monthly <- cbind(Monthly, monthlyReturn(ClosingPricesRead[,k])) # creating a matrix with all the monthly returns for each ticker
}
colnames(Monthly) <- TickerList
Monthly.Stock.Returns.2018[[j]] <- Monthly[End.Of.Month.Dates.2018[j]] #select the return for the particular date we need
...
Allocation.Matrix.2018[[j]] <- allocations[bestindex,]
}

Monthly.Net.Returns.2018 <- NULL
...
for(m in 1:12){ 
  Monthly.Stock.Net.Returns <- Allocation.Matrix.2018[[m]]*Monthly.Stock.Returns.2018[[m]]#calculating the return of our portfolio for every month by mulpipling the porpotion of each stock by its return for the month
  Monthly.Net.Returns.2018[m] <- sum(Monthly.Stock.Net.Returns) + 1 #sum up all the Monthly.Stock.Net.Returns for every month and create a list for the portfolio returns for each month
}

Final.Wealth.Without.TC.2018 <- prod(Monthly.Net.Returns.2018)

#multiplying the Monthly.Stock.Net.Returns of the portofolio we have the annual return of our portfolio
#sharpe ratio with 2 years window return 0.99, 4 years back 0.94, 1 year back 0.918, monthly 1.20, 10 days window for choosing stock and monthly for choosing the best portfolio return 1.38
```

```{r setup, include=FALSE}

Consecutive.Stocks.2018 <- list()
...
Consecutive.Stocks.2018[[j+1]] <- p

}
}
Consecutive.Stocks.2018 <- Consecutive.Stocks.2018[1:12]
# Delete the dummy number from the list
```

```{r}
Retained.Portfolio.Stocks <- rep(0, 12)
...
for (i in 1:12){
  if (length(Consecutive.Stocks.2018[[i]]) > 0) { # If statement checks if there are retained stocks. length(...) is the number of stocks that are retained.
    
    c <- 0
    for (j in 1:length(Consecutive.Stocks.2018[[i]])) {
      b <- Consecutive.Stocks.2018[[(i)]][,j]
      c <- c + min(Allocation.Matrix.2018[[(i-1)]][b],Allocation.Matrix.2018[[(i)]][b])
    }
...
for (i in 1:12){
  W.2018 = (W.2018*(1-Retained.Portfolio.Stocks[i])*0.9911 + W.2018*Retained.Portfolio.Stocks[i]) * Monthly.Net.Returns.2018[i]
  # (a + b)*c
...
Final.Wealth.2018 <- W.2018

print(Final.Wealth.2018)
